...
c          integer total
     ...
     >                   ,jv1,jv2,jv3,ju1,ju2,ju3,nar,jvol1,jgam
          ...
          integer gaselement(np), ierr
          real ratio
          pointer (lsump, lsum)
          pointer (totalloadp, totalload)
          ...
          enddo
           ...
           print *, nid, nelt, lelt, ntuple
           print *, 'print pload'
           if( nid .eq. 0) then!call printi(pload, nelt)
             do i =1, nelt
                print *, 'pload', i, pload(i)
             enddo
           endif 
           call izero(psum, lelt)
           call preSum(pload, psum, nelt)
           print *, 'print psum'
           if(nid .eq.0) call printi(psum, nelt)
c          print *, nid, nelt, lelt, 'psum(nelt): ', psum(nelt)
c          print *, 'ratio:', ratio
c          
c          lsum = psum(nelt)
c          print *, 'lsum:', lsum 
c          lsump = LOC(lsum)  
c          print *, 'lsump:', lsump 
c          totalloadp = MALLOC(1)
           stop
           call MPI_Scan(lsump, totalloadp, 1, 
     $        MPI_INTEGER, MPI_SUM, nekcomm, ierr) 
           totalloadp = LOC(totalload)
           print *, nid, totalload, lsum, loc(psum(nelt)),loc(totalload)
              
c          call izero(newgllnid, lelg) 

c            call ldblce(psum, nelgt, newgllnid, np_)

c         call bcast(newgllnid,4*nelgt)

c         call izero(trans, 3*lelg)
c         call track_elements(gllnid, newgllnid, nelgt, trans, 
c    $                               trans_n, lglel)
c         print *, 'print trans'
