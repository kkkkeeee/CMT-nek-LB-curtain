...
c preprocessing of interpolation step 
         ptinit_time_dum = dnekclock()
         call usr_particles_init
         ptinit_time = ptinit_time + dnekclock() - ptinit_time_dum
      endif
...
c compute_rhs_dt for the 5 conserved variables
         pttime_dum = dnekclock()
         call usr_particles_solver
         pttime = pttime + dnekclock() - pttime_dum
c        if (nid .eq. 0 .or. nid .eq. 30) then
...
c-----------------------------------------------------------------------
            ...
            enddo
         enddo
c        if (nid .eq. 0) print *, stage, "loop",
c    $         dnekclock()-timet
c        timet = dnekclock()
...
c        if (nid .eq. 0) print *, "compute_primitive_vars",
c    $         dnekclock()-timet
c        timet = dnekclock()
...
c dump out particle information. 
         ptio_time_dum = dnekclock()
         call usr_particles_io(istep)
         ptio_time = ptio_time + dnekclock() - ptio_time_dum
      end if
...
C> res1+=\f$\int_{\Gamma} \{\{\mathbf{A}^{\intercal}\nabla v\}\} \cdot \left[\mathbf{U}\right] dA\f$
      ium=(iu1-1)*nfq+iwm
      iup=(iu1-1)*nfq+iwp
      call   imqqtu(flux(iuj),flux(ium),flux(iup)) ! ccc
      call   imqqtu_dirichlet(flux(iuj),flux(iwm),flux(iwp)) ! ccc
      call igtu_cmt(flux(iwm),flux(iuj),graduf) ! [[u]].{{gradv}} ! ccc
      dumchars='after_igtu'
...
C> res1+=\f$\int_{\Gamma} \{\{\mathbf{A}\nabla \mathbf{U}\}\} \cdot \left[v\right] dA\f$
      call igu_cmt(flux(iwp),graduf,flux(iwm)) !ccc
...
!Finally add viscous surface flux functions of derivatives to res1.
         call surface_integral_full(res1(1,1,1,1,eq),flux(ieq)) !ccc
      enddo
      dumchars='end_of_rhs'
!     call dumpresidue(dumchars,999)
...
C> Compute coefficients for Runge-Kutta stages \cite{TVDRK}
      ...
      enddo
! TANIA'S CONSTRUCTED FIELD FOR PARTICLES
      call rzero(tcoef,9)
! TANIA'S CONSTRUCTED FIELD FOR PARTICLES
      return
      end
...
c we need our own IO features. Until then we use the default nek routines
     ...
     $                               .or.istep.eq.nstep)then
         dmtime1 = (ftime-pttime-ptinit_time)/istep
         dtime_ = glsum(dmtime1,1)
         if(nio.eq.0) write(6,*) 'fluid rhs compute time(Avg)  '
     $               ,dtime_/np

         dmtime1 = pttime/istep
         dtime_ = glsum(dmtime1,1)
         if(nio.eq.0) write(6,*) 'particle compute time(Avg)  '
     $               ,dtime_/np
      endif
      return 
      end
